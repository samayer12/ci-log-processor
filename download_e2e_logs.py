...
from github_api_calls import get_run_ids, get_all_job_ids
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("-1", "--once", action="store_true", 
                        help="Only fetch one page. Used with --page-size in"
                        "testing to avoid API rate-limits")
...
def get_workflow_id(repo, workflow_name, api):
    ...
    if workflow_id is None:
        ...
        print(f"Error: Workflow '{workflow_name}' "
              f"not found in repository {repo}")
